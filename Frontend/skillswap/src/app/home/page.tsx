  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      results = results.filter(professional => {
        ...
        if (professional.skillsOffered.some(skill => skill.toLowerCase().includes(query))) return true;
        
        // Search in skills wanted
        if (professional.skillsWanted.some(skill => skill.toLowerCase().includes(query))) return true;
    ...
    if (availabilityFilter && availabilityFilter !== "Availability") {
      results = results.filter(professional => professional.availability === availabilityFilter);
    }
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)]">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-8 text-[var(--color-text)]">Find Skilled Professionals</h1>
        ...
        <div className="space-y-6">
          {currentProfessionals.length > 0 ? (
            currentProfessionals.map(professional => (
              <div key={professional.id} className="bg-white rounded-lg p-6 shadow-md flex items-center justify-between border-l-4 border-[var(--color-main)]">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-green-600 mb-1">Skills Offered:</p>
                      <div className="flex space-x-2">
                        {professional.skillsOffered.map((skill, index) => (
                          <span key={index} className="bg-[var(--color-main)] text-white text-xs px-2 py-1 rounded-full">{skill}</span>
                        ))}
                      </div>
                      <p className="text-sm text-blue-600 mt-2 mb-1">Skill wanted:</p>
                      <div className="flex space-x-2">
                        {professional.skillsWanted.map((skill, index) => (
                          <span key={index} className="bg-[var(--color-highlight)] text-white text-xs px-2 py-1 rounded-full">{skill}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <div className="text-sm">
                    <span className="font-semibold">rating</span> {professional.rating}/5
                  </div>
                </div>
              </div>
            ))
              ...
              {[...Array(totalPages)].map((_, index) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2 border-t border-b border-gray-300 bg-white text-sm font-medium ${pageNumber === currentPage ? 'text-[var(--color-main)]' : 'text-gray-700'} hover:bg-gray-50`}
                  >
                    {pageNumber}
                  </button>
                );
